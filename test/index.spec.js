...
import { combineReducers, createActions, createState, RxStateProvider, connect } from '../index';

describe('RxState', () => {
  describe('combineReducers', () => {
    it('should combine reducers as expected', () => {
      const counterActions = createActions(['increment$', 'decrement$', 'reset$']);
      const CounterReducer$ = Rx.Observable.of(() => 0)
        .merge(
      ...
      const successRateActions = createActions(['succeed$', 'fail$', 'reset$']);
      const successRateReducer$ = Rx.Observable.of(() => ({
        successCount: 0,
        failCount: 0
      }))
        .merge(
